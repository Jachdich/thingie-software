...
void st7789_set_brightness(int brightness) {
    pwm_set_chan_level(st7789_cfg._slice_num, PWM_CHAN_A, brightness);
}
...
void st7789_init(const struct st7789_config* config, uint16_t width, uint16_t height)
{
    ...
    spi_init(st7789_cfg.spi, 65 * 1000 * 100);
    if (st7789_cfg.gpio_cs > -1) {
        spi_set_format(st7789_cfg.spi, 8, SPI_CPOL_0, SPI_CPHA_0, SPI_MSB_FIRST);
    } else {
        spi_set_format(st7789_cfg.spi, 8, SPI_CPOL_1, SPI_CPHA_1, SPI_MSB_FIRST);
    ...
    st7789_cfg._slice_num = pwm_gpio_to_slice_num(st7789_cfg.gpio_bl);
    pwm_set_wrap(st7789_cfg._slice_num, 31);
    st7789_set_brightness(15);
    pwm_set_enabled(st7789_cfg._slice_num, true);
